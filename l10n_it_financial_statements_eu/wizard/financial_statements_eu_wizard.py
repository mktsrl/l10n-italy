...
class FinancialStatementsEULog(models.TransientModel):
    _name = "financial.statements.eu.log"
    _description = "Unlinked account in Financial statements EU"
    financial_statements_id = fields.Many2one(
        "financial.statements.eu.wizard", string="Financial statements"
    )
...
class CreateFinancialStatementsWizard(models.TransientModel):
    _name = "financial.statements.eu.wizard"
    _description = "Wizard for Financial statements EU calculation"
    ...
    financial_statements_log_ids = fields.One2many(
        "financial.statements.eu.log",
    ...
    def _compute_period_data(self):
        for fs in self:
            ...
            fs.name = _("Financial statements EU")
    ...
    def financial_statements_eu_html_report(self):
        form_data = self.get_data()
        return self.env.ref(
            "l10n_it_financial_statements_eu.action_report_financial_statements_eu_xml"
    ...
    def cal_warnings_and_unlinked_acc(self, form_data):
        ...
        ].cal_financial_statements_eu_data(form_data)
        self.state = financial_statements_eu_data.get("financial_statements_state")
        self.log_warnings = "\n".join(financial_statements_eu_data.get("warnings"))

    def financial_statements_eu_xlsx_report(self):
        form_data = self.get_data()
        self.cal_warnings_and_unlinked_acc(form_data)
        return self.env.ref(
            "l10n_it_financial_statements_eu.action_report_financial_statements_eu_xlsx"
        ).report_action(self, data=form_data)

    def financial_statements_eu_xbrl_report(self):
        form_data = self.get_data()
        self.cal_warnings_and_unlinked_acc(form_data)
        return self.env.ref(
            "l10n_it_financial_statements_eu.action_report_financial_statements_eu_xbrl"
        ).report_action(self, data=form_data)
