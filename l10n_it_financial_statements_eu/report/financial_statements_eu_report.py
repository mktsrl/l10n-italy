...
class FinancialStatementsEuXlsxReport(models.AbstractModel):
    _name = "report.l10n_it_financial_statements_eu.fseu_xlsx_report"
    _description = "Export Financial statements EU in XLSX format"
    _inherit = "report.report_xlsx.abstract"

    def generate_xlsx_report(self, workbook, data, record_data):
        financial_statements_eu_data = self.env[
            "financial.statements.eu"
        ].cal_financial_statements_eu_data(data)
        sheet = workbook.add_worksheet(_("Financial statements EU"))
        ...
        max_l_descr = 0
        max_l_amount = 0
        for line in financial_statements_eu_data["financial_statements_eu_lines"]:
            code = line["code"]
...
class FinancialStatementsEuXBRLReport(models.AbstractModel):
    _name = "report.l10n_it_financial_statements_eu.fseu_xbrl_report"
    _description = "Export Financial statements EU in XBRL format"
    ...
    def get_financial_statements_line_tags(
        self,
        financial_statements_line_id,
        financial_statements_eu_lines,
        str_year,
        decimal_precision,
    ):
        result = ""
        for child in financial_statements_line_id.child_ids:
            result += self.get_financial_statements_line_tags(
                child, financial_statements_eu_lines, str_year, decimal_precision
            )
        if financial_statements_line_id.tag_xbrl:
            for line in financial_statements_eu_lines:
                if line["code"] == financial_statements_line_id["code"]:
                    amount = line["amount"]
                    break
            else:
                amount = None
            if amount is not None:
                result += self.get_xbrl_data_tag(
                    financial_statements_line_id.tag_xbrl,
                    str_year,
                    amount,
                    decimal_precision,
                )
    ...
    def generate_report(self, ir_report, docids, data=None):
        financial_statements_form_data = data
        financial_statements_eu_data = self.env[
            "financial.statements.eu"
        ].cal_financial_statements_eu_data(financial_statements_form_data)
        i_year = "i_" + str(financial_statements_form_data["year"])
        d_year = "d_" + str(financial_statements_form_data["year"])
        ...
        xbrl += """
            ...
            i_year,
            financial_statements_form_data["fiscalcode"],
            financial_statements_form_data["date_to"],
        )
        ...
        xbrl += """
            ...
            d_year,
            financial_statements_form_data["fiscalcode"],
            financial_statements_form_data["date_from"],
            financial_statements_form_data["date_to"],
        )
        xbrl += """
            <unit id="eur">
        ...
        xbrl += self.get_xbrl_data_tag(
            ...
            i_year,
            financial_statements_form_data["company_name"],
        )
        xbrl += self.get_xbrl_data_tag(
            "DatiAnagraficiSede",
            i_year,
            financial_statements_form_data["address"]
            + " - "
        ...
        xbrl += self.get_xbrl_data_tag(
            ...
            i_year,
            financial_statements_form_data["rea_capital"],
            0,
        )
        xbrl += self.get_xbrl_data_tag(
        ...
        xbrl += self.get_xbrl_data_tag(
            ...
            i_year,
            financial_statements_form_data["rea_office"],
        )
        xbrl += self.get_xbrl_data_tag(
            "DatiAnagraficiPartitaIva",
            i_year,
            financial_statements_form_data["vat_code"],
        )
        xbrl += self.get_xbrl_data_tag(
        ...
        xbrl += self.get_xbrl_data_tag(
            ...
            financial_statements_form_data["rea_office"]
            + " "
            + financial_statements_form_data["rea_num"],
        )
        ...
        if financial_statements_form_data["values_precision"] == "d":
            decimal_precision = 2
        else:
            decimal_precision = 0
        tmp_financial_statements_lines = self.env["financial.statements.eu"].search(
            [("code", "=", "PA")]
        )
        if len(tmp_financial_statements_lines) == 1:
            xbrl += self.get_financial_statements_line_tags(
                tmp_financial_statements_lines[0],
                financial_statements_eu_data["financial_statements_eu_lines"],
                i_year,
                decimal_precision,
            )
        tmp_financial_statements_lines = self.env["financial.statements.eu"].search(
...
class FinancialStatementsEUReportHTML(models.AbstractModel):
    _name = "report.l10n_it_financial_statements_eu.fseu_html_report"
    _description = "Export Financial statements EU in HTML format"

    def _get_report_values(self, docids, data):
        financial_statements_eu_data = self.env[
            "financial.statements.eu"
        ].cal_financial_statements_eu_data(data)
        return financial_statements_eu_data
